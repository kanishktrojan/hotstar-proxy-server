...
class StreamPrefetcher {
  ...
  async _pollVariant(variant) {
    ...
    try {
      ...
      // Count segments in this playlist for logging
      const segCount = (text.match(/^(?!#)[^\s]+/gm) || []).length;
      console.log(`[Prefetch][${this.slug}] Playlist ${variant.filename} polled (${segCount} segments)`);
    ...
    for (const seg of segments) {
      ...
      try {
        ...
        if (resp.ok) {
          ...
          this.rateLimiter.onCdnSuccess();
          newCount++;
          console.log(`[Prefetch][${this.slug}] ✓ ${segName} (${sizeMB}MB) | total: ${this.stats.segmentsPrefetched}`);
        } else {
          this.rateLimiter.onCdnError(resp.status);
          this.stats.errors++;
          console.warn(`[Prefetch][${this.slug}] ✗ ${segName} CDN ${resp.status}`);
        }
      } catch (err) {
        this.stats.errors++;
        console.warn(`[Prefetch][${this.slug}] ✗ ${segName} ${err.message}`);
      }
    }

...
class StreamPrefetcher {
  ...
  _launchVariantPollers() {
    this.variants.forEach((v, i) => {
      this._schedule(() => this._pollVariant(v), i * 200 + Math.random() * 200);
    });
  }
  ...
  async _pollVariant(variant) {
    ...
    // Re-poll slightly faster than segment duration (~1s for most live HLS)
    if (this.running) this._schedule(() => this._pollVariant(variant), 800 + Math.random() * 400);
  }
  ...
  async _processMediaPlaylist(text, baseDir) {
    ...
    for (const seg of toFetch) {
      ...
      // Tiny human-like delay between fetches (10-30ms)

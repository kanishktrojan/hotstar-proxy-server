...
class StreamPrefetcher {
  ...
  async _pollVariant(variant) {
    ...
    // Re-poll every 900-1300ms (human-like jitter around ~1s HLS refresh)
    if (this.running) {
      const jitter = 900 + Math.random() * 400;
    ...
    for (const seg of segments) {
      ...
      if (newCount > 0) {
        const delay = this.warming
          ? 20 + Math.random() * 40   // fast during warm-up
          : 40 + Math.random() * 80;  // normal pace

...
class RateLimiter {
  ...
  async acquire(priority = 'normal') {
    // Low-priority requests (prefetch) are skipped entirely when under CDN pressure
    // This preserves tokens for real viewer requests and avoids getting blocked
    if (priority === 'low' && this.backoffMultiplier > 1.5) {
      throw new Error('Skipped — rate limiter under pressure');
    }

    const needed = priority === 'high' ? 0.5 : priority === 'low' ? 1.5 : 1;
  ...
  onCdnError(statusCode) {
    ...
    if (statusCode === 429) {
      // Gentle backoff — don't double, just step up
      this.backoffMultiplier = Math.min(4, this.backoffMultiplier + 0.5);
      this.tokens = 0;
      console.warn(`[RateLimiter] 429 — backing off ${this.backoffMultiplier.toFixed(1)}x (${this.currentRate} req/s)`);
    } else if (statusCode === 403) {
      this.backoffMultiplier = Math.min(3, this.backoffMultiplier + 0.3);
